...
namespace kTools.Pooling
{
    public static class PoolingSystem
    {
        ...
        public static void CreatePool<T>(object key, T source, int instanceCount, bool destroyOnLoad = true)
        {
            // Test argument validity
            if (key == null)
            {
            ...
            // Create Pool
            var pool = new Pool<T>(key, source, instanceCount, destroyOnLoad, processor);
            s_Pools.Add(pool);
        }
